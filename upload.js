...
testModeButton.addEventListener('click', async function () {
  try {
    ...
    const files = await storage.listFiles('680d36a3002343bdd2ed');
    
    // Pick a random file
    const randomFile = files.files[Math.floor(Math.random() * files.files.length)];

    // Display the image and hide the rest
    ...
    if (diagnosis) {
      alert('You typed: ' + diagnosis);  // You can compare it to the correct diagnosis here.
    }
  } catch (error) {
